...
public class ProductController {
  ...
  //更新商品庫存
  @PutMapping("/{id}/stock")
  public ResponseEntity<?> updateProductStock(@PathVariable("id") int productId,
      @RequestBody Map<String, Integer> stock) {
    try {
      if (stock == null || !stock.containsKey("stock")) {
        return ResponseEntity.badRequest()
            .body(Map.of("message", "庫存數量不能為空"));
      }

      productService.updateProductStock(productId, stock.get("stock"));
      return ResponseEntity.ok()
          .body(Map.of("message", "庫存更新成功"));

    } catch (RuntimeException e) {
      return ResponseEntity.badRequest()
          .body(Map.of("message", e.getMessage()));
    } catch (Exception e) {
      log.error("更新庫存時發生錯誤", e);
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
          .body(Map.of("message", "更新庫存時發生錯誤"));
    }
  }
